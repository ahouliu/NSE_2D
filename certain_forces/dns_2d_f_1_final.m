...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [w,psi,Ek,Dk,Ik] = dns_2d_f_1_final(paras,d,e0,e1,f12_hat,Tf,w,psi)
tic;
...
Dk(Ninteration+1,1)         = 0;
Ik(Ninteration+1,1)         = 0;
...
for ind = 1:Ninteration
    ...
    Ek(ind+1) = E2;
    Dk(ind+1)         = (LxLy*nu)*norm(w_hat,'fro')^2*p_DFT ^2;
    Ik(ind+1)         = LxLy*sum(sum( f12_hat.*conj(psi_hat) ))*p_DFT ^2* g_func(t0+ind*dt,w_force);
end
%% return values(?)
w   = real(ifft2(w_hat));
